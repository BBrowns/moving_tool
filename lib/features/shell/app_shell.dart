...
class AppShell extends StatelessWidget {
  ...
  static const List<NavItem> navItems = [
    NavItem(path: '/dashboard', icon: Icons.dashboard_rounded, label: 'Dashboard'),
    NavItem(path: '/tasks', icon: Icons.check_circle_rounded, label: 'Taken'),
    NavItem(path: '/packing', icon: Icons.inventory_2_rounded, label: 'Inpakken'),
    NavItem(path: '/shopping', icon: Icons.shopping_cart_rounded, label: 'Shopping'),
    NavItem(path: '/expenses', icon: Icons.euro_rounded, label: 'Uitgaven'),
    NavItem(path: '/playbook', icon: Icons.book_rounded, label: 'Playbook'),
    NavItem(path: '/settings', icon: Icons.settings_rounded, label: 'Instellingen'), // Index 6
  ];
...
class _DesktopShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final railItems = navItems.take(6).toList();

    return Scaffold(
      body: Row(
        children: [
          NavigationRail(
            selectedIndex: navigationShell.currentIndex < railItems.length ? navigationShell.currentIndex : 0,
            ...
            trailing: Expanded(
              child: Align(
                ...
                child: Padding(
                  ...
                  child: isExtended 
                      ? TextButton.icon(
                          onPressed: () => navigationShell.goBranch(6),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: PageView(
        ...
        // Default PageScrollPhysics is typically clamping (Android) or bouncing (iOS). 
        // This provides the native "Physics" feel user wants.
        onPageChanged: (index) {
      ...
      bottomNavigationBar: NavigationBar(
        selectedIndex: widget.navigationShell.currentIndex < 5 ? widget.navigationShell.currentIndex : 0, 
        onDestinationSelected: (index) {
          widget.navigationShell.goBranch(index);
        },
        destinations: displayItems.map((item) => NavigationDestination(
          icon: Icon(item.icon),
          selectedIcon: Icon(item.icon, grade: 200),
          label: item.label,
        )).toList(),
