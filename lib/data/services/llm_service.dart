...
class LlmService {
  ...
  static Future<String> suggestPackingList(String roomType, String? geminiApiKey) async {
    final prompt = 'Maak een checklist van 5-10 essentiële items om in te pakken voor een: $roomType. Geef alleen de lijst met bullets.';
    return await _generateContent(prompt, geminiApiKey);
  }

  /// Generates moving advice based on days remaining
  static Future<String> getMovingAdvice(int daysLeft, String? geminiApiKey) async {
    final prompt = 'Ik ga verhuizen over $daysLeft dagen. Geef me 3 cruciale tips waar ik NU aan moet denken. Hou het kort en krachtig.';
    return await _generateContent(prompt, geminiApiKey);
  }

  static Future<String> _generateContent(String prompt, String? geminiApiKey) async {
    // Try Gemini first
  ...
  static Future<String> summarizeOverview(String markdownContent, String? geminiApiKey) async {
...
Je kunt het Ultiem Overzicht nog steeds zonder AI exporteren!''';
  ...
  static Future<String?> _tryOllama(String content) async {
    ...
    final prompt = '''Je bent een slimme assistent voor een verhuisapp. Analyseer dit verhuisoverzicht en geef een beknopte Nederlandse samenvatting.
